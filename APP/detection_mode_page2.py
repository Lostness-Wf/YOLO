...
class DetectionModePage2(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.log("检测模式二程序启动", "SUCCESS")
        self.log(f"检测模式二日志文件路径：{self.logger.log_file}")
    ...
    def selectModel(self):
        ...
        if file_path:
            try:
                self.log(f"检测模式二尝试加载模型: {file_path}")
                self.model = None
                ...
                self.log(f"检测模式二成功加载模型: {self.model_path}", "SUCCESS")
                QtWidgets.QMessageBox.information(
                    self, "检测模式二模型加载",
                    f"成功加载模型: {self.model_path}"
                )
            except Exception as e:
                self.log(f"检测模式二模型加载失败: {str(e)}", "ERROR")
    ...
    def detectImage(self):
        ...
        try:
            ...
            self.log("检测模式二图片检测完成", "SUCCESS")

        except Exception as e:
            self.log(f"检测模式二检测失败: {str(e)}", "ERROR")
            QtWidgets.QMessageBox.critical(
                self, "检测模式二错误",
                f"检测失败: {str(e)}"
            )
    ...
    def showImage(self, label, image):
        try:
            ...
            self.log(f"检测模式二图片显示失败: {str(e)}", "ERROR")

    def testFunction(self):
        self.log("检测模式二测试按钮被点击", "INFO")